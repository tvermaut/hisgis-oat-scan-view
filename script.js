...
function getOppervlakHTML(opp){
    ...
    if(opp > 0){
        ...
        e_h.setAttribute("class","ps-0 text-start border-start-0 mono");
        e_h.innerHTML = (e == 0 && r == 0 && b == 0 ? "" : String(e).padStart(2,'0'));
        let r_h = document.createElement("td");
        r_h.setAttribute("class","px-0 text-center border-start-0 border-end-0 mono");
        r_h.innerHTML = (r == 0 && b == 0 ? "" : ( b == 0 ? r : String(r).padStart(2,'0')));
        let b_h = document.createElement("td");
        b_h.setAttribute("class","pe-0 text-end border-end-0 mono");
        b_h.innerHTML = (b == 0 ? "" : b);
        h.push(b_h);
        h.push(r_h);
        h.push(e_h);
...
function getBedragHTML(b){
    ...
    if(b > 0){
        ...
        g_h.setAttribute("class","pe-0 text-end border-end-0 mono");
        g_h.innerHTML = (g == 0 ? '' : g);
        h.push(g_h);
        h.push(c_h);
    } else {
